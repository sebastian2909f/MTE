...
for ind, rad_prox in enumerate(rad_prox):
    ...
    for nodo1 in ['Nodo97', 'Nodo89', 'Nodo34', 'Nodo32']:
        for nodo2 in ['Nodo97', 'Nodo89', 'Nodo34', 'Nodo32']:
            if nodo1 != nodo2:
                distancia = calcular_distancia((G.nodes[nodo1]["latitud"], G.nodes[nodo1]["longitud"]),
                                             (G.nodes[nodo2]["latitud"], G.nodes[nodo2]["longitud"]))            
                G.add_edge(nodo1, nodo2, distancia=distancia)
    ...
    if rad_prox == 0:
        val_del = [65, 66, 73, 82] #R=0.25  
    if rad_prox == 0.25:
        val_del = [65, 66, 73, 82] #R=0.25
    
    if rad_prox == 0.5:
        val_del = [65, 66, 73, 8, 9, 14, 15, 29 ,82] #R=0.5
        
    if rad_prox == 0.75:
        val_del = [65, 66, 73, 8, 9, 14, 15, 6, 45, 61, 82, 29] #R=0.75
    ...
    df_ubi = pd.read_csv(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\DATOS\UBICACIONES_MAPA-RES.csv')
    # print(df_ubi.iloc[:, 0])
    ind = 0
    ...
    '''
    ED, LAS BANDERAS	NODO 32
    ED. TORRES TUNDAMA	NODO 34
    TORRES SAN ANDRES	NODO 89
    VALPARAISO	NODO 97
    '''
    # ini = 4*24*30*5
    # inter = 4*24*30*1
    ...
    n41m_df = pd.read_csv(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\DATOS\NODOS_MAPA_RES.csv', usecols=columnas_seleccionadas).iloc[ini:inter]
    ...
    if ima == 1:
        ...
        # axs[0].plot(n4_pot_act.iloc[:,'NODO92']) 
        axs[0].plot(n4_pot_act.loc[:, 'NODO32'])
        axs[1].plot(n4_pot_act.loc[:, 'NODO34'])
        axs[2].plot(n4_pot_act.loc[:, 'NODO89'])
        axs[3].plot(n4_pot_act.loc[:, 'NODO97'])
        axs[4].plot(n4_pot_act.loc[:, 'NODO4'])
        
        axs[0].set_title('ED, LAS BANDERAS')
        axs[1].set_title('ED. TORRES TUNDAMA')
        axs[2].set_title('TORRES SAN ANDRES')
        axs[3].set_title('VALPARAISO')
    ...
    if rad_prox == 0.25:
        ...
        n4_pot_act['NODO34'] = n4_pot_act['NODO34'].clip(lower=0.2, upper=2.2)
        # n4_pot_act['NODO38'] = n4_pot_act['NODO38'].clip(lower=, upper=)
        n4_pot_act['NODO52'] = n4_pot_act['NODO52'].clip(lower=0, upper=5.7)
        n4_pot_act['NODO53'] = n4_pot_act['NODO53'].clip(lower=0, upper=0.9)
        n4_pot_act['NODO54'] = n4_pot_act['NODO54'].clip(lower=0, upper=3.4)
        # n4_pot_act['NODO56'] = n4_pot_act['NODO56'].clip(lower=, upper=)
        # n4_pot_act['NODO57'] = n4_pot_act['NODO57'].clip(lower=, upper=)
        n4_pot_act['NODO64'] = n4_pot_act['NODO64'].clip(lower=8, upper=31)
        # n4_pot_act['NODO70'] = n4_pot_act['NODO70'].clip(lower=, upper=)
        # n4_pot_act['NODO81'] = n4_pot_act['NODO81'].clip(lower=, upper=)
        n4_pot_act['NODO83'] = n4_pot_act['NODO83'].clip(lower=0.9, upper=2.6)
        n4_pot_act['NODO89'] = n4_pot_act['NODO89'].clip(lower=0, upper=2.2)
        # n4_pot_act['NODO90'] = n4_pot_act['NODO90'].clip(lower=, upper=)
        # n4_pot_act['NODO91'] = n4_pot_act['NODO91'].clip(lower=, upper=)
        n4_pot_act['NODO97'] = n4_pot_act['NODO97'].clip(lower=0, upper=0.5)
        
    #R = [[ 4  5  7 13 16 17 20 21 22 23 27 28 30 32 34 38 44 51 52 53 54 56 57 64 70 79 80 81 83 89 90 91 92 97]]
    if rad_prox == 0.5:
        
        n4_pot_act['NODO4'] = n4_pot_act['NODO4'].clip(lower=0, upper=2.3)
        # n4_pot_act['NODO5'] = n4_pot_act['NODO5'].clip(lower=, upper=)   #2 
        # n4_pot_act['NODO7'] = n4_pot_act['NODO7'].clip(lower=, upper=)
        n4_pot_act['NODO13'] = n4_pot_act['NODO13'].clip(lower=2.1, upper=11.3)    #4
        n4_pot_act['NODO16'] = n4_pot_act['NODO16'].clip(lower=0, upper=1.4)    #5
        n4_pot_act['NODO17'] = n4_pot_act['NODO17'].clip(lower=0, upper=4)
        n4_pot_act['NODO20'] = n4_pot_act['NODO20'].clip(lower=0, upper=8.2)
        # n4_pot_act['NODO21'] = n4_pot_act['NODO21'].clip(lower=, upper=)    #8
        n4_pot_act['NODO22'] = n4_pot_act['NODO22'].clip(lower=1.4, upper=4)    #9
        n4_pot_act['NODO23'] = n4_pot_act['NODO23'].clip(lower=0, upper=12.1)    #10
        # n4_pot_act['NODO27'] = n4_pot_act['NODO27'].clip(lower=0, upper=)       #11
        n4_pot_act['NODO28'] = n4_pot_act['NODO28'].clip(lower=0, upper=3.25)
        n4_pot_act['NODO30'] = n4_pot_act['NODO30'].clip(lower=0, upper=6)    #13
        n4_pot_act['NODO32'] = n4_pot_act['NODO32'].clip(lower=0, upper=3.4)
        n4_pot_act['NODO34'] = n4_pot_act['NODO34'].clip(lower=0.2, upper=2.2)
        # n4_pot_act['NODO38'] = n4_pot_act['NODO38'].clip(lower=, upper=)
        n4_pot_act['NODO44'] = n4_pot_act['NODO44'].clip(lower=9.2, upper=30.3)       #17
        # n4_pot_act['NODO51'] = n4_pot_act['NODO51'].clip(lower=, upper=)    #18
        n4_pot_act['NODO52'] = n4_pot_act['NODO52'].clip(lower=0, upper=5.7)
        n4_pot_act['NODO53'] = n4_pot_act['NODO53'].clip(lower=0, upper=0.9)
        n4_pot_act['NODO54'] = n4_pot_act['NODO54'].clip(lower=0, upper=3.4)
        # n4_pot_act['NODO56'] = n4_pot_act['NODO56'].clip(lower=, upper=)
        ...
        n4_pot_act['NODO64'] = n4_pot_act['NODO64'].clip(lower=8, upper=31)
        # n4_pot_act['NODO70'] = n4_pot_act['NODO70'].clip(lower=, upper=)
        # n4_pot_act['NODO79'] = n4_pot_act['NODO79'].clip(lower=, upper=)        #26
        n4_pot_act['NODO80'] = n4_pot_act['NODO80'].clip(lower=0.3, upper=3.2)       #27
        # n4_pot_act['NODO81'] = n4_pot_act['NODO81'].clip(lower=, upper=)
        n4_pot_act['NODO83'] = n4_pot_act['NODO83'].clip(lower=0.9, upper=2.6)
        n4_pot_act['NODO89'] = n4_pot_act['NODO89'].clip(lower=0, upper=2.2)
        # n4_pot_act['NODO90'] = n4_pot_act['NODO90'].clip(lower=, upper=)
        # n4_pot_act['NODO91'] = n4_pot_act['NODO91'].clip(lower=, upper=)
        # n4_pot_act['NODO92'] = n4_pot_act['NODO92'].clip(lower=, upper=)    #33
        n4_pot_act['NODO97'] = n4_pot_act['NODO97'].clip(lower=0, upper=0.5)
        
 #    R = 0.75, [ 4  5  7 10 11 12 13 16 17 18 20 21 22 23 27 28 30 32 33 34 35 38 42 44
 # 46 50 51 52 53 54 56 57 60 62 64 70 71 79 80 81 83 89 90 91 92 93 94 95
 # 96 97]  
    ...
    if rad_prox == 0.75:
        ...
        # n4_pot_act['NODO7'] = n4_pot_act['NODO7'].clip(lower=, upper=)
        n4_pot_act['NODO10'] = n4_pot_act['NODO10'].clip(lower=0, upper=6.4)    #4
        n4_pot_act['NODO11'] = n4_pot_act['NODO11'].clip(lower=0, upper=4.8)    #5
        n4_pot_act['NODO12'] = n4_pot_act['NODO12'].clip(lower=0, upper=6)    #6
        n4_pot_act['NODO13'] = n4_pot_act['NODO13'].clip(lower=2.1, upper=11.3)    #
        n4_pot_act['NODO16'] = n4_pot_act['NODO16'].clip(lower=0, upper=1.4)    #
        n4_pot_act['NODO17'] = n4_pot_act['NODO17'].clip(lower=0, upper=4)
        n4_pot_act['NODO18'] = n4_pot_act['NODO18'].clip(lower=0, upper=12.7)    #10
        n4_pot_act['NODO20'] = n4_pot_act['NODO20'].clip(lower=0, upper=8.2)
        ...
        n4_pot_act['NODO32'] = n4_pot_act['NODO32'].clip(lower=0, upper=3.4)
        n4_pot_act['NODO33'] = n4_pot_act['NODO33'].clip(lower=0, upper=12.7)    #19
        n4_pot_act['NODO33'] = n4_pot_act['NODO35'].clip(lower=0, upper=2.2)    #20
        n4_pot_act['NODO34'] = n4_pot_act['NODO34'].clip(lower=0.2, upper=2.2)
        # n4_pot_act['NODO38'] = n4_pot_act['NODO38'].clip(lower=, upper=)
        n4_pot_act['NODO42'] = n4_pot_act['NODO42'].clip(lower=0, upper=1.2)    #23
        n4_pot_act['NODO44'] = n4_pot_act['NODO44'].clip(lower=9.2, upper=30.3)       #
        n4_pot_act['NODO46'] = n4_pot_act['NODO46'].clip(lower=13, upper=41.8)    #25
        n4_pot_act['NODO50'] = n4_pot_act['NODO50'].clip(lower=0, upper=1.9)    #26
        # n4_pot_act['NODO51'] = n4_pot_act['NODO51'].clip(lower=, upper=)    #
        n4_pot_act['NODO52'] = n4_pot_act['NODO52'].clip(lower=0, upper=5.7)
        n4_pot_act['NODO53'] = n4_pot_act['NODO53'].clip(lower=0, upper=0.9)
        n4_pot_act['NODO54'] = n4_pot_act['NODO54'].clip(lower=0, upper=3.4)
        # n4_pot_act['NODO56'] = n4_pot_act['NODO56'].clip(lower=, upper=)
        # n4_pot_act['NODO57'] = n4_pot_act['NODO57'].clip(lower=, upper=)
        n4_pot_act['NODO60'] = n4_pot_act['NODO60'].clip(lower=0, upper=0.4)    #33
        n4_pot_act['NODO62'] = n4_pot_act['NODO62'].clip(lower=3.3, upper=6.8)    #34
        n4_pot_act['NODO64'] = n4_pot_act['NODO64'].clip(lower=8, upper=31)
        # n4_pot_act['NODO70'] = n4_pot_act['NODO70'].clip(lower=, upper=)
        # n4_pot_act['NODO71'] = n4_pot_act['NODO71'].clip(lower=, upper=)    #37
        ...
        n4_pot_act['NODO93'] = n4_pot_act['NODO93'].clip(lower=0, upper=30.8)    #46
        n4_pot_act['NODO94'] = n4_pot_act['NODO94'].clip(lower=0.4, upper=4.3)
        n4_pot_act['NODO95'] = n4_pot_act['NODO95'].clip(lower=0, upper=60.7)
        n4_pot_act['NODO96'] = n4_pot_act['NODO96'].clip(lower=0, upper=11.3)
        n4_pot_act['NODO97'] = n4_pot_act['NODO97'].clip(lower=0, upper=0.5)
    ...
    if ima == 1:
        ...
        fig1, axs = plt.subplots(4,1,figsize=(12,8))
        # axs[0].plot(n4_pot_act.iloc[:,'NODO92']) 
        axs[0].plot(n4_pot_act_nor.loc[:, 'NODO32'])
        axs[1].plot(n4_pot_act_nor.loc[:, 'NODO34'])
        axs[2].plot(n4_pot_act_nor.loc[:, 'NODO89'])
        axs[3].plot(n4_pot_act_nor.loc[:, 'NODO97'])
...
#%% INDICES DE NODOS OBJETIVOS
    ...
    n1_ind = n4_pot_act_nor.columns.get_loc('NODO32')
    n2_ind = n4_pot_act_nor.columns.get_loc('NODO34')
    n3_ind = n4_pot_act_nor.columns.get_loc('NODO89')
    n4_ind = n4_pot_act_nor.columns.get_loc('NODO97')
        ...
        for ind, cell in enumerate(CELL):
            ...
            for epoch in tqdm(range(epocas)):
                ...
                for times in range(len(feature_train_set)):
                    ...
                    # if rad_prox == 0:
                    #     y_hat_p = y_hat[ [-4, -3, -2, -1] ]
                    #     y_train_set_p = y_train_set[times, [-4, -3, -2, -1]]
                    # elif rad_prox == 0.25:
                    #     y_hat_p = y_hat[[5, 6, -4, -1]]
                    #     y_train_set_p = y_train_set[times, [5, 6, -4, -1]]
                    # elif rad_prox == 0.5:
                    #     y_hat_p = y_hat[[13, 14, -5, -1]]
                    #     y_train_set_p = y_train_set[times, [13, 14, -5, -1]]
                    # elif rad_prox == 0.75:
                    #     y_hat_p = y_hat[[17, 19, -9, -1]]
                    #     y_train_set_p = y_train_set[times, [17, 19, -9, -1]]
                ...
                with torch.no_grad():
                    ...
                    for val_time in range(len(feature_val_set)):
                        ...
                        # if rad_prox == 0:
                        #     val_y_hat_p = val_y_hat[ [-4, -3, -2, -1] ]
                        #     y_val_set_p = y_val_set[val_time, [-4, -3, -2, -1]]
                        # elif rad_prox == 0.25:
                        #     val_y_hat_p = val_y_hat[[5, 6, -4, -1]]
                        #     y_val_set_p = y_val_set[val_time, [5, 6, -4, -1]]
                        # elif rad_prox == 0.5:
                        #     val_y_hat_p = val_y_hat[[13, 14, -5, -1]]
                        #     y_val_set_p = y_val_set[val_time, [13, 14, -5, -1]]
                        # elif rad_prox == 0.75:
                        #     val_y_hat_p = val_y_hat[[17, 19, -9, -1]]
                        #     y_val_set_p = y_val_set[val_time, [17, 19, -9, -1]]
                    ...
                    if val_loss < best_val_loss:
                        ...
                        epochs_without_improvement = 0
                        torch.save(model.state_dict(), r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\RESIDENCIAS\\'+out_txt+'.pth')
                        print('-------------> BEST MODEL')
                    else:
            ...
            if ima == 1:
                ...
                plt.savefig(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\RESIDENCIAS\FIGURAS\Train_'+out_txt+'.png')
            ...
            path = r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\RESIDENCIAS\\'+out_txt+'.pth'
            # print(path)
            ...
            axs[0].set_title('BANDERAS MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[0],mae_list[0],r2_list[0]))
            axs[1].set_title('TUNDAMA MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[1],mae_list[1],r2_list[1]))
            axs[2].set_title('SAN ANDRES MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[2],mae_list[2],r2_list[2]))
            axs[3].set_title('VALPARAISO MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[3],mae_list[3],r2_list[3]))
            ...
            # Guardar la figura en un archivo (puedes especificar el formato, como PNG, PDF, SVG, etc.)
            plt.savefig(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\RESIDENCIAS\FIGURAS\Res_'+out_txt+'.png')
            ...
            # input_seq_length = 4*24*30
            # output_seq_length = 4*24
            nombre_archivo = r"C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\RESIDENCIAS\METRICAS\DATA_"+mod+"_OUT"+str(output_seq_length)+"_IN"+str(input_seq_length)+".csv"
