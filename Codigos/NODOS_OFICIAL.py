...
for ind, rad_prox in enumerate(rad_prox):
    ...
    #  print(ubi)
      #print(ubi[0])
      #print(ubi[1])
      G.add_node("Nodo"+str(i), latitud = -ubi[0], longitud = -ubi[1])
    ...
    if rad_prox == 0:
        val_del = [61, 62] #R=0.25    
    if rad_prox == 0.25:
        val_del = [61, 62] #R=0.25
    
    if rad_prox == 0.5:
        val_del = [61, 62, 0, 3, 16] #R=0.
        
    if rad_prox == 0.75:
        val_del = [61, 62, 0, 3, 16, 66, 65, 73, 45, 8, 97] #R=0.5
    ...
    df_ubi = pd.read_csv(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\DATOS\UBICACIONES_MAPA.csv')
    # print(df_ubi.iloc[:, 0])
    ind = 0
    ...
    n41m_df = pd.read_csv(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\DATOS\NODOS_MAPA_UNIVER.csv', usecols=columnas_seleccionadas).iloc[ini:inter]
    ...
    #%% FIGURA
    #R=0.5 [4  6 10 17 21 29 32 42 44 70 92 93 94 95 96]
    #44, 
    
    '''R = 0.75, [ 4  6  7 10 17 18 20 21 23 28 29 32 34 38 42 43 44 46 53 54 56 57 64 70
     71 81 82 83 90 91 92 93 94 95 96]'''
    if ima == 1:
        ...
        axs[2].plot(n4_pot_act.loc[:, 'NODO95'])
        axs[3].plot(n4_pot_act.loc[:, 'NODO96'])
        axs[4].plot(n4_pot_act.loc[:, 'NODO91'])
    ...
    if rad_prox == 0.75:
        ...
        # n4_pot_act['NODO7'] = n4_pot_act['NODO7'].clip(lower=,upper=)             #3
        ...
        # n4_pot_act['NODO38'] = n4_pot_act['NODO38'].clip(lower=0,upper=3.4) #14 <-------
        ...
        # n4_pot_act['NODO56'] = n4_pot_act['NODO56'].clip(lower=0,upper=0.96) #21
        # n4_pot_act['NODO57'] = n4_pot_act['NODO57'].clip(lower=0,upper=3) #22
        n4_pot_act['NODO64'] = n4_pot_act['NODO64'].clip(lower=8.09,upper=30.93) #23
        # n4_pot_act['NODO70'] = n4_pot_act['NODO70'].clip(lower=0,upper=3) #24
        # n4_pot_act['NODO71'] = n4_pot_act['NODO71'].clip(lower=8.09,upper=30.93) #25
        # n4_pot_act['NODO81'] = n4_pot_act['NODO81'].clip(lower=8.09,upper=30.93) #26
        ...
        # n4_pot_act['NODO90'] = n4_pot_act['NODO90'].clip(lower=0.09,upper=6.65) #29
        # n4_pot_act['NODO91'] = n4_pot_act['NODO91'].clip(lower=0.98,upper=2.66) #30
    ...
    if ima == 1:
        ...
        axs[2].plot(n4_pot_act.loc[:, 'NODO95'])
        axs[3].plot(n4_pot_act.loc[:, 'NODO96'])
        axs[4].plot(n4_pot_act.loc[:, 'NODO46'])
        # axs[4].plot(n4_pot_act.loc[:, 'NODO44'])
    ...
    if ima == 1:
        fig1, axs = plt.subplots(4,1,figsize=(12,8))
        # axs[0].plot(n4_pot_act.iloc[:,'NODO92']) 
        axs[0].plot(n4_pot_act_nor.loc[:, 'NODO92'])
        axs[1].plot(n4_pot_act_nor.loc[:, 'NODO93'])
        axs[2].plot(n4_pot_act_nor.loc[:, 'NODO95'])
        axs[3].plot(n4_pot_act_nor.loc[:, 'NODO96'])
        # axs[4].plot(n4_pot_act_nor.loc[:, 'NODO94'])
    ...
    for ind, k in enumerate(K):
        for ind, cell in enumerate(CELL):
            ...
            for epoch in tqdm(range(epocas)):
                ...
                for times in range(len(feature_train_set)):
                    ...
                    if rad_prox == 0:
                        ...
                        y_hat_p = y_hat[ [-5, -4, -2, -1] ]
                        y_train_set_p = y_train_set[times, [-5, -4, -2, -1]]
                ...
                with torch.no_grad():
                    ...
                    for val_time in range(len(feature_val_set)):
                        ...
                        if rad_prox == 0:
                            val_y_hat_p = y_hat[ [-4, -3, -2, -1] ]
                            y_val_set_p = y_train_set[times, [-4, -3, -2, -1]]
                        else:
                            val_y_hat_p = val_y_hat[ [-5, -4, -2, -1] ]
                            y_val_set_p = y_val_set[val_time, [-5, -4, -2, -1]]
                    ...
                    if val_loss < best_val_loss:
                        ...
                        epochs_without_improvement = 0
                        torch.save(model.state_dict(), r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\UNIVERSIDADES\RAD_2\\'+out_txt+'.pth')
                        print('-------------> BEST MODEL')
                    else:
            ...
            if ima == 1:
                ...
                plt.savefig(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\UNIVERSIDADES\RAD_2\FIGURAS\Train_'+out_txt+'.png')
            ...
            path = r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\UNIVERSIDADES\RAD_2\\'+out_txt+'.pth'
            # print(path)
            ...
            for i, data in enumerate(y_out_test):
                ...
                # print(i)
                if rad_prox == 0:
                    node_0_test = np.append(node_0_test,data[-4])
                    node_1_test = np.append(node_1_test,data[-3])
            ...
            for i, data in enumerate(y_out_pre):
                # print(data)
                if rad_prox == 0:
                    node_0 = np.append(node_0,data[-4])
                    node_1 = np.append(node_1,data[-3])
            ...
            axs[0].set_title('U COOP MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[0],mae_list[0],r2_list[0]))
            axs[1].set_title('UNIMAR MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[1],mae_list[1],r2_list[1]))
            axs[2].set_title('UDENAR MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[2],mae_list[2],r2_list[2]))
            axs[3].set_title('VIPRI MSE = {:.5f} - MAE = {:.5f} - R² = {:.5f}'.format(mse_list[3],mae_list[3],r2_list[3]))
            ...
            # Guardar la figura en un archivo (puedes especificar el formato, como PNG, PDF, SVG, etc.)
            plt.savefig(r'C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\UNIVERSIDADES\RAD_2\FIGURAS\Res_'+out_txt+'.png')
            ...
            # output_seq_length = 4*24
            nombre_archivo = r"C:\Users\GIIEE\Desktop\MTE_GRAPHS\MODELOS\UNIVERSIDADES\RAD_2\METRICAS\DATA_"+mod+"_OUT"+str(output_seq_length)+"_IN"+str(input_seq_length)+".csv"
